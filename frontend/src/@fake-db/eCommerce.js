...
// GET: Return products
// ------------------------------------------------
mock.onGet('/ecommerce/products').reply(config => {
...
// ------------------------------------------------
mock.onGet(/\/ecommerce\/products\/\d+/).reply(config => {
...
// ------------------------------------------------
mock.onGet('/ecommerce/wishlist').reply(() => {
...
// ------------------------------------------------
mock.onGet('/ecommerce/cart').reply(() => {
...
// ------------------------------------------------
mock.onPost('/ecommerce/cart').reply(config => {
...
// ------------------------------------------------
mock.onDelete(/\/ecommerce\/cart\/\d+/).reply(config => {
  // Get product id from URL
  let productId = config.url.substring(config.url.lastIndexOf('/') + 1)
...
// ------------------------------------------------
mock.onPost('/ecommerce/wishlist').reply(config => {
  // Get product from post data
  const { productId } = JSON.parse(config.data)
